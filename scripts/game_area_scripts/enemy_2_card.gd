...
func scale_hand_dimensions(scale_factor: float):
	...
	# Update the offset to maintain right-side positioning
	offset_left = -new_width * 0.7  # Adjusted for better positioning
...
func scale_label(scale_factor: float):
	...
	if player_label:
		...
		var label_width = 62.0 * scale_factor
		var label_height = 23.0 * scale_factor
		
		player_label.size = Vector2(label_width, label_height)
		
		# Adjust position for rotation
		var label_offset_top = 31.1 * scale_factor
		player_label.position = Vector2(0, label_offset_top)
		player_label.size = Vector2(label_width, label_height)
